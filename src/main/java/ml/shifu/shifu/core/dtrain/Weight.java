...
public class Weight {
    ...
    private static final float DEFAULT_INITIAL_UPDATE = 0.1f;
    private static final float DEFAULT_MAX_STEP = 50f;
    ...
    public Weight(int numWeight, double numTrainSize, double rate, String algorithm, double reg, RegulationLevel rl,
        ...
        this.learningDecay = (float) learningDecay;
            ...
            switch(this.rl) {
                ...
                case L1:
                    if(Double.compare(this.reg, 0d) == 0) {
                        ...
                        weights[i] = Math.signum(delta) * Math.max(0, Math.abs(delta) - shrinkValue);
                    }
                    break;
                case L2:
                default:
    ...
    public float[] calculateWeights(float[] weights, float[] gradients, int iteration) {
        if(this.updateRule != null) {
                ...
                switch(this.rl) {
                    ...
                    case L1:
                        if(Double.compare(this.reg, 0d) == 0) {
                            ...
                            float shrinkValue = (float) (this.reg / getNumTrainSize());
                            float delta = updateWeight(i, weights, gradients);
                            weights[i] = Math.signum(delta) * Math.max(0, Math.abs(delta) - shrinkValue);
                        }
                        break;
                    case L2:
    ...
    private float updateWeight(int index, float[] weights, float[] gradients) {
        if(this.algorithm.equalsIgnoreCase(DTrainUtils.BACK_PROPAGATION)) {
            return updateWeightBP(index, weights, gradients);
        } else if(this.algorithm.equalsIgnoreCase(DTrainUtils.QUICK_PROPAGATION)) {
            return updateWeightQBP(index, weights, gradients);
    ...
    private float updateWeightBP(int index, float[] weights, float[] gradients) {
        double delta = (gradients[index] * this.getLearningRate()) + (this.lastDelta[index] * this.getMomentum());
        this.lastDelta[index] = delta;
        return (float) delta;
    }

    private float updateWeightQBP(int index, float[] weights, float[] gradients) {
        final float w = weights[index];
        final double d = this.lastDelta[index];
        final float s = -gradients[index] + this.decay * w;
        final double p = -lastGradient[index];
        float nextStep = 0f;
    ...
    private float updateWeightMHP(int index, float[] weights, float[] gradients) {
        if(Math.abs(gradients[index]) < ZERO_TOLERANCE) {
            return 0;
        } else if(gradients[index] > 0) {
            return this.getLearningRate();
    ...
    private float updateWeightSCG(int index, float[] weights, float[] gradients) {
        throw new RuntimeException("SCG propagation is not supported in distributed NN computing.");
    }

    private float updateWeightRLP(int index, float[] weights, float[] gradients) {
        // multiply the current and previous gradient, and take the sign. We want to see if the gradient has changed its
        // sign.
        final int change = DTrainUtils.sign(gradients[index] * lastGradient[index]);
        double weightChange = 0;
     ...
     */
    public void setLearningDecay(float learningDecay) {
