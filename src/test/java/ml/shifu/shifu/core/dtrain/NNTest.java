...
public class NNTest {
    ...
    public void testNnApp() throws IOException {
        ...
        props.setProperty(CommonConstants.SHIFU_MODEL_CONFIG, getClass().getResource("/dttest/config/nn/ModelConfig.json")
                .toString());
        props.setProperty(CommonConstants.SHIFU_COLUMN_CONFIG,
                getClass().getResource("/dttest/config/nn/ColumnConfig.json").toString());

        props.setProperty(GuaguaConstants.GUAGUA_INPUT_DIR, getClass().getResource("/dttest/data/").toString());

        GuaguaUnitDriver<NNParams, NNParams> driver = new GuaguaMRUnitDriver<NNParams, NNParams>(
                props);
